...
for testCase in spec-test sample-f sample-e sample-d sample-c; do 
    ./zoo --mode OPTTSP < $testCase.txt > out.txt
    diff <(head -n 1 $testCase-OPTTSP-out.txt) <(head -n 1 out.txt)
done
